...
阅读记录（Library）会自动保存每本书 / 每份文档的阅读痕迹与进度（EPUB 精确到章节内位置），支持一键收藏；被删除的便签与阅读记录会进入回收站，可随时恢复或彻底删除；便签、阅读记录、回收站数据均可跨设备**双向同步**（云端权威）。
...
| Library | 阅读记录（`{serial}.json`） | 每本书 / 每份文档的阅读痕迹 |
| Recycle | 回收站（便签 / 阅读记录） | 删除 = 移入回收站，可恢复；彻底删除 = 云端同步清除 |

- **删除语义**：删除 = 进回收站（本地移入 Recycle 目录 + 云端跨区移动，不复制）；彻底删除 = 本地删除 + 云端回收站区删除。
- **换设备**：阅读记录标题与进度上云，恢复后原文件令牌失效时弹“资源不存在”对话框，默认保留记录。
- **回收站清单**（`recycle.json`）双向同步，换设备后可恢复被删除的便签与记录。
- **网络文档正文**仅缓存于本地（`Cache/*.txt`），不同步云端，恢复后重新抓取。
...
- 密钥托管分三种状态：**首次使用**（生成新密钥）、**无密码模式**（明文备份密钥，免密采用）、**密码包裹**（密钥经个人密码派生密钥加密后托管服务端）。
- 更换设备后首次进入便签本需输入个人密码解锁；忘记密码将**永久丢失**便签数据（设计行为）。
- 忘记密码时支持"重新开始"流程：二次确认后设置新密码并重新生成密钥；旧设备上未备份密钥的便签无法解密（视为孤儿数据，提供删除清理提示，防误删）。
