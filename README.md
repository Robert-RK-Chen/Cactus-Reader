...
重大更新：客户端与服务器采用**零知识（Zero-Knowledge）**加密设计：

| 层面 | 技术 | 说明 |
| --- | --- | --- |
| 便签内容加密 | AES-256-GCM 认证加密 | 每次加密使用随机 nonce，认证标签保证密文完整性 |
| 个人密码存储 | PBKDF2-SHA256 | 符合 OWASP 密码存储建议，恒定时间比较防计时攻击 |
| 跨设备密钥 | 密码包裹密钥 | 便签密钥 K 由"个人密码派生密钥 KEK"加密后托管服务端；服务端仅持有盐与密文密钥，无法解密便签数据 |
...
- 技术栈：ASP.NET Core Minimal API（`net10.0`），由旧版 WCF 服务迁移而来（2026-08）
...
- 建表脚本：`cactus_reader.sql`（含 user / code / userkey / vaultkey 四张表）

# � 部署与配置

## 服务端敏感配置（不提交真实值）

`appsettings.json` 中数据库密码与 Microsoft Graph 凭据已替换为占位符（`YOUR_*`），**严禁将真实凭据提交到 Git**。本地运行请通过**环境变量**注入真实配置（优先级高于 appsettings.json）：
...
- 便签内容使用 AES-256-GCM 加密；个人密码使用 PBKDF2-SHA256（600,000 次迭代 + 随机盐）。
...
> Copyright © 2022-2026, Robert Chen. catom technology.
